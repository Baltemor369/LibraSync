...
class UI(tk.Tk):
    ...
    def display_library(self):
        ...
        modification_B = tk.Button(frame_bottom,
                                   text="New Manga",
                                   width=9,
                                   relief="raised",
                                   command=self.display_modification
        ...
        back_button = tk.Button(frame_bottom,
                                width=9,
                                text="Return",
    ...
    def research_bar(self, evt):
        ...
        self.display_tab(self.frame_body,
                         MangaLib(self.all_manga.find_manga(search_word))
                         )
    ...
    def modification(self, ID:int):
        ...
        if data_verify:
            if ID != -1:
                ...
                for elt in find_VN(volume_nb_get):
                    self.all_manga.add_manga(Manga(self.all_manga.get_new_id(), name_get, author_get, type_get, int(elt), description_get, float(valuation_get)))
                    self.all_manga.save_data()
    ...
    def display_stats(self):
        ...
        # self.set_geometry(300,300+50*len(self.all_manga.get()))
...
def find_VN(string:str) -> list[int]:
    list_volume_number = []
    buffer = string.split(",")
    for elt in buffer:
        if elt.find("-") == -1:
